...
<title>SIKSIN &mdash;member_delete</title>
...
<script type="text/javascript">
  console.log("디티오", "${siksinMemberDTO.password}");
  //console.log("입력값", $("#password").val());
</script>

<script type="text/javascript">	
	function memberUpdate() {
		if (confirm("${siksinMemberDTO.memberName}님 회원정보를 수정 하시겠습니까?")) {
			location.href = "./SiksinMemberUpdate?memberNum=${siksinMemberDTO.memberNum}"
		}
	}
</script>

<script type="text/javascript">
	function passwordCheckMember() {
		if (${siksinMemberDTO.password} != $("#password").val()) {
			alert("비밀번호가 일치하지 않습니다. 다시 입력해주세요.");
		}else {
			$("#memberDelete1").submit();
		}
	}
</script>
</head>
						...
						<div class="row row-mt-15em">
							<div class="col-md-12 mt-text animate-box"
								...
								<span class="intro-text-small"> <a href="index.jsp"
									target="_blank">대한민국 No.1 맛집 서비스 siksin. 회원정보탈퇴</a></span>
								<h1 class="cursive-font">SIKSIN.회원정보탈퇴</h1>
							</div>
						</div>
					</div>
				</div>
				...
				<div class="row" id="member#ffffff">
					<div id="buttonUpdatDeleteview" class="mx-auto">
						...
						<ul class="authcd_y" id="authcd_y_all">
							<li class="on col-md-6" id="authcd_y2" onclick="memberUpdate()"><a>회원정보변경</a></li>
							<li class="false col-md-6" id="authcd_y1"><a>회원탈퇴</a></li>
						</ul>
					</div>
					<fieldset name="member" class="member">
						<h3 class="fieldset_h3">회원정보탈퇴</h3>
						...
						<form method="post" enctype="application/x-www-form-urlencoded"
							...
							<div class="mp_member_out">
								...
								<ol>
									<li><strong>해당 휴대폰번호로 30일간 재가입 불가능</strong><br> <span>회원탈퇴를
											신청하시면 해당 닉네임은 즉시 탈퇴 처리되며 기존에 사용하던 휴대폰번호로는 30일 동안 재가입이 불가합니다.</span></li>
									<li><strong>회원정보 및 게시물 삭제</strong><br> <span>회원탈퇴
											즉시 아래에 해당하는 개인정보가삭제됩니다.<br> 개인정보 : 이메일 계정, 비밀번호, 휴대폰번호,
											생일, 성별 정보 삭제
									</span></li>
								</ol>
							</div>
							...
							<div class="form-group">
								<div class="member_button">
									<input type="reset" class="btn btn-primary" id="update_reset"
										onclick="location.href='./index.jsp'" value="취소하기"> <input
										class="btn btn-primary" id="submitButton"
										onclick="passwordCheckMember()" value="탈퇴하기">
								</div>
							</div>
						</form>
					</fieldset>
